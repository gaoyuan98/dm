...
func (sf *statFlusher) writeAndFlush(logs []string, startOff int, l int) {
	...
	for i := startOff; i < startOff+l; i++ {
		...
		sf.buffer.Dm_build_1575(bytes, 0, len(bytes))

		if sf.buffer.Dm_build_1554() >= FLUSH_SIZE {
			sf.doFlush(sf.buffer)
		}
	}

	if sf.buffer.Dm_build_1554() > 0 {
		sf.doFlush(sf.buffer)
	}
}

func (sf *statFlusher) doFlush(buffer *Dm_build_1549) {
	if sf.needCreateNewFile() {
		sf.closeCurrentFile()
		sf.logFile = sf.createNewFile()
	}
	if sf.logFile != nil {
		buffer.Dm_build_1569(sf.logFile, buffer.Dm_build_1554())
	}
}
